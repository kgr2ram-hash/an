// Fetch live gold/silver prices and convert to INR
// Uses multiple sources for accuracy

// Today's actual rates (March 25, 2026 - Coimbatore)
// 22K: ~₹13,600/g, 24K: ~₹14,837/g, Silver: ~₹105/g
const FALLBACK = {
  gold: {
    usd_oz: 3050,
    inr_gram_24k: 14837,
...
export async function fetchGoldSilverPrices() {
  const cached = sessionStorage.getItem('gold_silver_v4')
  if (cached) {
    try {
      const c = JSON.parse(cached)
      if (Date.now() - c._ts < 1800000) return c // 30 min cache
    } catch {}
  }
  ...
  try {
    // Fetch metals + exchange rate in parallel
    ...
    if (gold && silver) {
      ...
      // Indian gold prices include ~3-5% premium over international (import duty + GST)
      const indiaPremium = 1.04 // 4% premium
      ...
      prices = {
        ...
        silver: {
          usd_oz: silver.price,
          // India silver: international + ~18% import duty + 3% GST + margins ≈ 1.25x, then retail adds more
          // Current ratio: ₹235 actual / ₹93 intl ≈ 2.53x (includes all taxes + retail markup)
  ...
  sessionStorage.setItem('gold_silver_v4', JSON.stringify(prices))
  return prices
}
